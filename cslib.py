...
if __name__ == "__main__":
    ...
    # code to find monthly revenue by country
    ts_new = {}
    for key, item in ts_all.items():
        print(key, item.shape)
        if key != 'all':
            ts_temp = ts_all[key].groupby('year_month').aggregate({'revenue': 'sum'})
            ts_temp = pd.DataFrame.to_dict(ts_temp)['revenue']
            for key2, item2 in ts_temp.items():
                if key2 not in ts_new.keys():
                    ts_new[key2] = {key: item2}
                else:
                    ts_new[key2][key] = item2

    ts_df = pd.DataFrame.from_dict(ts_new, orient='index')
    print(ts_df.head())
    ...
    testx, testy, testdates = engineer_features(ts_all['eire'], training=False)

    print("testx = ", testx)
    print("testy = ", testy)
    print("dates = ", testdates)